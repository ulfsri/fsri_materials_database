...
df = pd.DataFrame(columns = ['Wet_cp', 'Dry_cp', 'Wet_k', 'Dry_k', 'STA_MLR', 'CONE_MLR_25', 'CONE_MLR_50', 'CONE_MLR_75', 'CONE_HRRPUA_25', 'CONE_HRRPUA_50', 'CONE_HRRPUA_75', 'CO_Yield', 'MCC_HRR', 'Soot_Yield', 'MCC_HoC', 'Cone_HoC', 'HoR', 'HoG', 'MCC_Ign_Temp', 'Melting_Temp', 'Emissivity', 'Full_JSON'])
...
for d in sorted((f for f in os.listdir(data_dir) if not f.startswith(".")), key=str.lower):
    if os.path.isdir(f'{data_dir}/{d}'):
        ...
        if os.path.isfile(f'material_headers/{material}_header.json'):
            ...
            em_list = []
            if os.path.isfile(f'{data_dir}{material}/FTIR/IS/{material}_Emissivity_Table.html'):
                try:
                ...
                em_list.append(f'\t\t"table": "FTIR/IS/{material}_Emissivity_Table.html"\n')
