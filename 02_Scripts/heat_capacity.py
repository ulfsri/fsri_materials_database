            ...
            for d_ in os.scandir(f'{d.path}/STA/N2/'):
                ...
                for f in glob.iglob(f'{d_.path}/*.csv'):
                    ...
                    if 'Meta' in f or '.DS_Store' in f:
                        ...
                        temp_df['Temp'] = data_temp_df.filter(regex='Temp', axis='columns')
                        temp_df['time'] = data_temp_df.filter(regex='Time', axis='columns')
                        temp_df['Mass'] = data_temp_df.filter(regex='Mass', axis='columns')
                        temp_df['DSC'] = data_temp_df.filter(regex='DSC', axis='columns')
                        temp_df['Mass'] = temp_df['Mass']/temp_df.loc[0,'Mass']
